...
function buildLinks(tasks: readonly TaskRow[]): Link[] {
  ...
  for (const monster of new Set(
    tasks.map((task) => task.monster).filter((m) => m !== null),
  )) {
    links.push({
      kind: "monster",
      source: monster,
      url: monsterWikiUrl(monster),
    });
  }
...
async function main(): Promise<void> {
  ...
  console.log(
    `Checking ${links.length} links (${titles.length} distinct pages) on the wiki`,
  );
  ...
  console.log(
    `  ${redirected} reached through a redirect (fine -- the wiki follows its own)`,
  );
