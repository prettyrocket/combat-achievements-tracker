...
describe("sanitize: wikilinks", () => {
  it("renders the label of a piped link", () => {
    expect(
      sanitize(
        "Clear the [[Jewelled_Crab|Crystal Crabs]] room without wasting an orb.",
      ),
    ).toBe("Clear the Crystal Crabs room without wasting an orb.");
  });
  ...
  it("renders the target of an unpiped link", () => {
    ...
    ).toBe("Defeat Sol Heredit after using Fortis Salute to the north.");
  });

  it("handles a link inside the league region field", () => {
    expect(
      sanitize("Asgarnia&Kourend,Asgarnia,(with [[Grimoire]] Relic)"),
    ).toBe("Asgarnia&Kourend,Asgarnia,(with Grimoire Relic)");
  });
...
describe("assertClean", () => {
  ...
  it("names the field in the error, so the failure is actionable", () => {
    expect(() => assertClean("see [[Thing]]", "task #99 description")).toThrow(
      /task #99 description/,
    );
  });
});
...
describe("toRow", () => {
  ...
  it("maps display casing onto the domain types and derives points", () => {
    const result = toRow(
      row({ tier: "Grandmaster", type: "Kill Count" }),
      noCompletion,
    );
  ...
  it('maps the "None" monster placeholder to null', () => {
    ...
    expect(toRow(row({ monster: "Zulrah" }), noCompletion).monster).toBe(
      "Zulrah",
    );
  });
  ...
  it("treats an absent league region as null", () => {
    ...
    expect(
      toRow(row({ league_region: "Kourend" }), noCompletion).leagueRegion,
    ).toBe("Kourend");
  });

  it("joins the completion percentage, or null when there is none", () => {
    expect(toRow(row({ id: 3 }), new Map([[3, 41.9]])).completionPct).toBe(
      41.9,
    );
  ...
  it("fails loudly on an unknown tier", () => {
    expect(() => toRow(row({ tier: "Legendary" }), noCompletion)).toThrow(
      /unknown tier/,
    );
  });

  it("fails loudly on an unknown task type", () => {
    expect(() => toRow(row({ type: "Vibes" }), noCompletion)).toThrow(
      /unknown task type/,
    );
  });

  it("fails on a description that sanitizes away to nothing", () => {
    expect(() => toRow(row({ task: "<sup>x</sup>" }), noCompletion)).toThrow(
      /empty after/,
    );
  });
});
...
describe("validate", () => {
  ...
  it("rejects duplicate ids, since ids are the join key", () => {
    ...
    expect(() => validate(buildTasks(rows, new Map()), new Map())).toThrow(
      /Duplicate task id/,
    );
  });
  ...
  it("rejects a completion entry with no matching task", () => {
    const tasks = buildTasks(validFixture(), new Map());
    expect(() => validate(tasks, new Map([[99999, 10]]))).toThrow(
      /unknown task id/,
    );
  });
