...
export function toRow(
  ...
  if (leagueRegion !== null)
    assertClean(leagueRegion, `${where} league region`);
...
export function buildTasks(
  ...
  return rows
    .map((row) => toRow(row, completion))
    .sort((a, b) => a.wikiId - b.wikiId);
}
...
export function validate(
  ...
  for (const t of tasks) {
    if (seen.has(t.wikiId))
      fail(`Duplicate task id ${t.wikiId} -- ids are the join key.`);
    seen.add(t.wikiId);
  }
  ...
  if (orphans.length > 0) {
    fail(
      `completion.json references ${orphans.length} unknown task id(s): ${orphans.join(", ")}`,
    );
  }
}
