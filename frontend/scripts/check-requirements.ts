...
async function bucketQuery<T>(query: string): Promise<T[]> {
  const res = await fetch(BUCKET + encodeURIComponent(query), {
    headers: { "User-Agent": UA },
  });
  if (!res.ok) {
    throw new Error(
      `Bucket query failed: HTTP ${res.status} ${res.statusText}\n  ${query}`,
    );
  }
  const body = (await res.json()) as { bucket?: T[]; error?: string };
  if (body.error)
    throw new Error(`Bucket query failed: ${body.error}\n  ${query}`);
  if (!Array.isArray(body.bucket)) {
    throw new Error(
      `Bucket query returned no rows array -- did the API shape change?\n  ${query}`,
    );
  }
  return body.bucket;
}
...
async function loadMonsters(): Promise<string[]> {
  ...
  return [
    ...new Set(
      tasks.map((task) => task.monster).filter((m): m is string => m !== null),
    ),
  ].sort();
}
...
function wikiSlayerFor(monster: string, levels: Map<string, number>): number {
  ...
  return Math.max(
    0,
    ...names.map((name) => levels.get(name.toLowerCase()) ?? 0),
  );
}
...
async function main(): Promise<void> {
  ...
  console.log(
    `Checking ${gatedMonsters().length} gated monsters against the wiki`,
  );

  // --- the Slayer half, in both directions ----------------------------------

  const monsterRows = await bucketAll<MonsterRow>(
    "'name','slayer_level'",
    "infobox_monster",
  );
