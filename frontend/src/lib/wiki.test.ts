...
describe('wiki URLs', () => {
  ...
  it('encodes an apostrophe rather than leaving it raw in the URL', () => {
    expect(monsterWikiUrl("Kree'arra")).toBe("https://oldschool.runescape.wiki/w/Kree'arra")
  })
...
describe('splitAtColon', () => {
  it('leaves a name with no colon alone', () => {
    expect(splitAtColon('Vardorvis Sleeper')).toEqual(['Vardorvis Sleeper', null])
  })

  it('splits at the first colon and trims the tail', () => {
    expect(splitAtColon('Chambers of Xeric: CM (5-Scale) Speed-Chaser')).toEqual([
      'Chambers of Xeric',
      'CM (5-Scale) Speed-Chaser',
    ])
  })
