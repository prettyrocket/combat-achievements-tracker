...
import { parseFull, parseTasks, RuneProfileError, syncedLabel } from '@/lib/runeprofile'
...
function full(over: Record<string, unknown> = {}) {
  return {
    ...
    quests: [
      { id: 6, name: 'Priest in Peril', points: 1, type: 'free', state: 'finished' },
      { id: 7, name: 'Dragon Slayer II', points: 5, type: 'members', state: 'in_progress' },
      { id: 8, name: 'Monkey Madness II', points: 4, type: 'members', state: 'not_started' },
    ],
    combatAchievements: [
...
describe('parseTasks', () => {
  ...
  it('ignores rows that are not shaped like a task', () => {
    const body = { data: [{ index: 0, completed: true }, null, { completed: true }, 7] }
...
describe('parseFull', () => {
  ...
  it('normalises the space-separated timestamp instead of trusting Date', () => {
    expect(parseFull(full({ updatedAt: '2026-01-05 00:00:00.000000' })).updatedAt?.toISOString()).toBe(
...
describe('syncedLabel', () => {
  ...
  it('reads as a phrase after "synced"', () => {
    expect(syncedLabel(new Date('2026-08-02T04:00:00.000Z'), now)).toBe('synced today')
    expect(syncedLabel(new Date('2026-08-01T04:00:00.000Z'), now)).toBe('synced yesterday')
    expect(syncedLabel(new Date('2026-07-20T12:00:00.000Z'), now)).toBe('synced 13 days ago')
  })

  it('switches to months once days stop being informative', () => {
    expect(syncedLabel(new Date('2026-01-02T12:00:00.000Z'), now)).toBe('synced 7 months ago')
  })
