...
function player(skills: Record<string, { level: number }>, rest: Record<string, unknown> = {}) {
  return {
    username: 'lynx titan',
...
describe('parseWomPlayer', () => {
  ...
  it('drops levels below 1, which are WOM saying "no data"', () => {
    const parsed = parseWomPlayer(player({ slayer: { level: 92 }, sailing: { level: -1 } }))
    expect(parsed.levels).toEqual({ Slayer: 92 })
  })

  it('ignores entries that are not shaped like a skill', () => {
    const parsed = parseWomPlayer(
      player({ slayer: { level: 92 }, mining: null, farming: { level: 'x' } } as never),
    )
    expect(parsed.levels).toEqual({ Slayer: 92 })
  })
  ...
  it('survives a missing updatedAt rather than inventing a date', () => {
    const parsed = parseWomPlayer(player({ slayer: { level: 92 } }, { updatedAt: null }))
  ...
  it('reports NO_SNAPSHOT when there is no snapshot', () => {
    expect(codeOf(() => parseWomPlayer(player({}, { latestSnapshot: null })))).toBe('NO_SNAPSHOT')
  })

  it('reports NO_SNAPSHOT when the snapshot holds nothing usable', () => {
    expect(codeOf(() => parseWomPlayer(player({ overall: { level: 2278 } })))).toBe('NO_SNAPSHOT')
  })
...
describe('updatedLabel', () => {
  ...
  it('reads as a phrase after "updated"', () => {
    expect(updatedLabel(new Date('2026-08-02T04:00:00.000Z'), now)).toBe('updated today')
    expect(updatedLabel(new Date('2026-08-01T04:00:00.000Z'), now)).toBe('updated yesterday')
    expect(updatedLabel(new Date('2026-07-20T12:00:00.000Z'), now)).toBe('updated 13 days ago')
  })

  it('switches to months once days stop being informative', () => {
    expect(updatedLabel(new Date('2026-01-02T12:00:00.000Z'), now)).toBe('updated 7 months ago')
  })
