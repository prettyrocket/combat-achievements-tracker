...
describe("hostile stored values", () => {
  it("drops junk, unknown and duplicate ids", async () => {
    const seed = JSON.stringify({
      completed: [1, 9999, -5, "x", null, 2.5, 2, 2],
    });
    const { store } = await loadStore({ seed });
...
describe("storage that fails", () => {
  it("degrades to memory when storage is blocked outright", async () => {
    ...
    expect(store.getStorageError()).toEqual(
...
describe("export and import", () => {
  ...
  it.each([
    ["malformed JSON", "{{{"],
    [
      "a file from another app",
      JSON.stringify({ app: "wikisync", completed: [1] }),
    ],
    [
      "a missing completed list",
      JSON.stringify({ app: "combat-achievements-tracker" }),
    ],
...
describe("undo", () => {
  ...
  it("takes back a reset and a WikiSync-style replace", async () => {
    ...
    store.undo();
    expect([...store.getCompleted()].sort((a, b) => a - b)).toEqual([
      18, 42, 315,
    ]);
    store.undo();
    expect(store.getCompleted().size).toBe(0);
...
describe("sanitizeIds", () => {
  ...
  it("treats a non-array as nothing at all", async () => {
    const { store } = await loadStore();
    expect(store.sanitizeIds("nope", new Set([1]))).toEqual({
      ids: [],
      dropped: 0,
    });
  });
