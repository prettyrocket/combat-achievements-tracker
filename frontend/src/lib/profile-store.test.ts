...
async function load(seed?: unknown) {
  ...
  if (seed !== undefined) storage.map.set('ca-tracker:profile:v1', JSON.stringify(seed))
...
describe('sanitizeLevels', () => {
  it('keeps a plain map of levels', async () => {
    ...
    expect(store.sanitizeLevels({ Slayer: 92, Ranged: 70 })).toEqual({ Slayer: 92, Ranged: 70 })
  })
  ...
  it.each([['null', null], ['an array', [1, 2]], ['a string', 'nope']])(
...
describe('sanitizeQuests', () => {
  it('dedupes by normalized name, keeping the first spelling', async () => {
    const { store } = await load()
    expect(store.sanitizeQuests(['Regicide', ' regicide ', 'Dragon Slayer II'])).toEqual([
      'Regicide',
      'Dragon Slayer II',
    ])
  })
  ...
  it('drops blanks and non-strings', async () => {
    const { store } = await load()
    expect(store.sanitizeQuests(['Regicide', '', '   ', 7, null])).toEqual(['Regicide'])
  })
})
...
describe('the store', () => {
  it('starts empty and reads back what it was given', async () => {
    ...
    store.setProfile({ levels: { Slayer: 92 }, quests: ['Regicide'] }, 'wikisync')
    expect(store.getProfile()).toEqual({ levels: { Slayer: 92 }, quests: ['Regicide'] })
    expect(store.getSource()).toBe('wikisync')
  })

  it('persists, and survives a reload', async () => {
    const first = await load()
    first.store.setProfile({ levels: { Slayer: 92 }, quests: ['Regicide'] }, 'wikisync')
    const saved = JSON.parse(first.storage.map.get('ca-tracker:profile:v1')!)

    const second = await load(saved)
    expect(second.store.getProfile()).toEqual({ levels: { Slayer: 92 }, quests: ['Regicide'] })
    expect(second.store.getSource()).toBe('wikisync')
  })

  it('sanitizes what it reads off disk, not just what it is handed', async () => {
    const { store } = await load({ levels: { Slayer: 0, Ranged: 70 }, quests: ['a', 'A'] })
    expect(store.getProfile()).toEqual({ levels: { Ranged: 70 }, quests: ['a'] })
  })
