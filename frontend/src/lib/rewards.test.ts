...
const ONE_EACH = ['EASY', 'MEDIUM', 'HARD', 'ELITE', 'MASTER', 'GRANDMASTER'].map((t) =>
  task(t as Tier),
)
...
describe('rewardTiers', () => {
  ...
  it('matches the point requirements the wiki publishes', () => {
    expect(rewardTiers(TASKS).map((t) => t.required)).toEqual([41, 161, 419, 1075, 1945, 2671])
  })
})
...
describe('projectRewards', () => {
  ...
  it('names every tier crossed when a plan clears more than one', () => {
    expect(projectRewards(TIERS_1_EACH, 0, 10).unlocks.map((t) => t.tier)).toEqual([
      'EASY',
      'MEDIUM',
      'HARD',
      'ELITE',
  ...
  it('leaves out tiers already unlocked', () => {
    expect(projectRewards(TIERS_1_EACH, 10, 5).unlocks.map((t) => t.tier)).toEqual(['MASTER'])
  })
