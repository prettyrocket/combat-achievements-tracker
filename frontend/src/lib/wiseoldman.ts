...
export function parseWomPlayer(body: unknown): WomLookup {
  if (body === null || typeof body !== "object") {
    throw new WomLookupError(
      "BAD_RESPONSE",
      "Wise Old Man's reply wasn't a player.",
    );
  }
  ...
  if (Object.keys(levels).length === 0) {
    throw new WomLookupError(
      "NO_SNAPSHOT",
      "Wise Old Man returned no levels for that name.",
    );
  }
 ...
 */
export async function fetchWomLevels(
  rsn: string,
  signal?: AbortSignal,
): Promise<WomLookup> {
  const name = displayRsn(rsn);
  if (name === "")
    throw new WomLookupError("BAD_NAME", "Enter your RuneScape name.");
  ...
  if (response.status === 404) {
    throw new WomLookupError(
      "NOT_TRACKED",
      `Wise Old Man has never tracked ${name}.`,
    );
  }
  ...
  if (!response.ok) {
    throw new WomLookupError(
      "BAD_RESPONSE",
      `Wise Old Man answered ${response.status}.`,
    );
  }
  ...
  try {
    ...
    throw new WomLookupError(
      "BAD_RESPONSE",
      "Wise Old Man's reply wasn't JSON.",
    );
  }
