...
describe("the assumption the format rests on", () => {
  ...
  it("has 655 tasks with contiguous ids from 0", () => {
    expect(ALL_IDS).toHaveLength(655);
    expect(ALL_IDS[0]).toBe(0);
    expect(ALL_IDS[654]).toBe(654);
...
describe("round trip", () => {
  ...
  it("survives a realistic set of completions", () => {
    const completed = [0, 1, 7, 8, 63, 64, 100, 321, 653, 654];
    const out = decodeShareCode(encodeShareCode({ completed, list: [] }));
  ...
  it("survives every task being complete", () => {
    const out = decodeShareCode(
      encodeShareCode({ completed: ALL_IDS, list: [] }),
    );
    expect(out.completed).toEqual(ALL_IDS);
  });

  // The byte boundaries are where an off-by-one hides: bit 7 is the last of
  // byte 0, bit 654 is the last real bit of the last byte.
  it.each([0, 7, 8, 15, 16, 647, 648, 653, 654])(
    "survives task %i alone",
    (id) => {
      expect(
        decodeShareCode(encodeShareCode({ completed: [id], list: [] }))
          .completed,
      ).toEqual([id]);
    },
  );
...
describe("the profile", () => {
  ...
  it("round-trips levels and quests", () => {
    const out = decodeShareCode(
      encodeShareCode({ completed: [], list: [], profile }),
    );
    expect(out.profile.levels).toEqual(profile.levels);
    expect([...out.profile.quests].sort()).toEqual([
      "Priest in Peril",
      "Song of the Elves",
    ]);
  ...
  it("clamps a level past the ceiling rather than overflowing its byte", () => {
    const out = decodeShareCode(
      encodeShareCode({
        completed: [],
        list: [],
        profile: { levels: { Slayer: 9000 }, quests: [] },
      }),
    );
  ...
  it("matches a skill whatever case it arrived in", () => {
    const out = decodeShareCode(
      encodeShareCode({
        completed: [],
        list: [],
        profile: { levels: { sLaYeR: 70 }, quests: [] },
      }),
    );
  ...
  it("writes no section at all for an empty profile", () => {
    ...
    expect(
      encodeShareCode({ completed: [1, 2], list: [3], profile: null }),
    ).toBe(bare);
  });

  it("reads a code with no profile section, exactly as it used to", () => {
    // Hand-built: version, bitset, list length, no tail.
  ...
  it("rejects a profile section that was cut off mid-flight", () => {
    ...
    expect(() =>
      decodeShareCode(asCode(bytes.slice(0, bytes.length - 2))),
  ...
  it("skips skills and quests appended by a newer build, and counts them", () => {
    ...
    const bytes = new Uint8Array(84 + 1 + 25 + 1 + 5);
    bytes[0] = 2;
    bytes[83] = 0;
    bytes[84] = 25;
    bytes[84 + 1 + 18] = 92; // Slayer, position 18, known
    bytes[84 + 1 + 23] = 70; // Sailing, position 23, known
    bytes[84 + 1 + 24] = 55; // appended skill, unknown
    bytes[84 + 1 + 25] = 5;
    bytes[84 + 1 + 25 + 1] = 0b1000_0000; // quest 7: Priest in Peril
    bytes[84 + 1 + 25 + 1 + 4] = 0b0000_0001; // quest 32: past the end
    const out = decodeShareCode(asCode(bytes));
...
describe("what cannot travel in a link", () => {
  ...
  it("reports nothing lost for a profile that fits", () => {
    expect(
      profileWireLoss({ levels: { Slayer: 92 }, quests: ["Regicide"] }),
    ).toEqual({
      levels: 0,
      quests: 0,
    });
  ...
  it("agrees with what the codec actually drops", () => {
    ...
    expect(out.profile.quests).toHaveLength(
      profile.quests.length - loss.quests,
    );
  });
});

describe("size and shape", () => {
  // 84 bytes -> 112 characters: the 82-byte bitset plus the version and length
  // bytes that make it a format rather than a blob. 84 divides by 3, so this
  // one encodes with no padding to strip.
  it("is 112 characters when it carries completions only", () => {
    const code = encodeShareCode({ completed: ALL_IDS, list: [] });
    expect(code).toHaveLength(112);
    // The point of a bitset: a full browser and an empty one cost the same.
    expect(encodeShareCode({ completed: [], list: [] })).toHaveLength(112);
  });
  ...
  // 29 bytes for the section -- 1 + 23 levels + 1 + 4 quest bytes -- on top of
  // the 84-byte header: 151 characters, 39 more than without, whether the
  // profile is one level or all of them.
  it("adds a fixed 39 characters for a profile, full or nearly empty", () => {
...
describe("hostile and malformed input", () => {
  ...
  it("rejects a code whose task list claims more entries than it carries", () => {
    ...
    bytes[0] = 2;
    bytes[83] = 10; // says ten ids follow; none do
  ...
  it("names both versions when the format does not match", () => {
    const bytes = new Uint8Array(84);
    bytes[0] = 9;
    expect(() => decodeShareCode(asCode(bytes))).toThrow(/format 9.*reads 2/);
  });

  it("refuses a v1 code rather than reading its length byte as bitset", () => {
    // The Mad Angel release widened the bitset, so every byte after it moved.
    // A v1 code is not recoverable here, but it must fail loudly, not decode
    // onto the wrong tasks with a task list built from whatever followed.
    const bytes = new Uint8Array(83);
    bytes[0] = 1;
    expect(() => decodeShareCode(asCode(bytes))).toThrow(/format 1.*reads 2/);
  });

  it("ignores the padding bits past task 654", () => {
    // Every bit set, including 655, which is padding in the last byte.
  ...
  it("drops out-of-range completions on the way in instead of corrupting a neighbour", () => {
    const out = decodeShareCode(
      encodeShareCode({ completed: [5, 655, -1, 1e9, 6], list: [] }),
    );
  ...
  it("truncates a task list too long to state its own length", () => {
    ...
    expect(
      decodeShareCode(encodeShareCode({ completed: [], list: long })).list,
    ).toHaveLength(255);
  });
});
